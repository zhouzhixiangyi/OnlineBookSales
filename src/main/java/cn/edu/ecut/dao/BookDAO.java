...
public class BookDAO {
    ...
    public  boolean save( Book b) {
        ...
        //System.out.println("INSERT INTO books ( bookName , price , picture , number , introduction , category , sno , author ) VALUES ("+b.getBookName()+","+b.getPrice()+","+b.getPicture()+","+","+b.getNumber()+","+b.getIntroduction()+","+b.getCategory()+","+b.getSales().getId()+","+b.getAuthor()+")");
    ...
    public Book find(int id){
        String sql = "SELECT bookID , bookName , price , picture , number , introduction , category , sno , author FROM books WHERE bookID= ?";
        return this.find(sql , (Object)id);
    }
    ...
    public Book find(String name){
        String sql = "SELECT bookID , bookName , price , picture , number , introduction , category , sno , author FROM books WHERE bookName = ?";
    ...
    public Book find(String name , int sno){
        String sql = "SELECT bookID , bookName , price , picture , number , introduction , category , sno , author FROM books WHERE bookName = ? and sno = ?";
        Book b = null;
        JdbcHelper h = JdbcHelper.getInstance();
        ResultSet rs = h.query(sql, name , sno);
        try {
            if(rs.next()){
                b = this.wrap(rs);
            }
    ...
    public boolean addNumber(int bno , int num){
        JdbcHelper h = JdbcHelper.getInstance();
        String sql = "UPDATE books SET number =number + num Where bookID = ? ";
        int count = h.update(sql, bno);
        h.release();
        return count == 1 ;
    }
    ...
    public List<Book> findBySID(int sid) {

        List<Book> Books = new ArrayList<>();

        String SQL = "SELECT bookID , bookName , price , picture , number , introduction , category , sno , author FROM books WHERE sno = ?" ;

        JdbcHelper h = JdbcHelper.getInstance();
        ResultSet rs = h.query( SQL,sid);
        try {
            while (rs.next()) {
                Book b = this.wrap(rs);
                Books.add(b);
